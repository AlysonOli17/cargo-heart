...
export const Route = createFileRoute("/porto-operacao")({
  head: () => ({ meta: [{ title: "Operação Porto — Frota Busato" }] }),
  component: () => <AppLayout><PortoOperacaoPage /></AppLayout>,
});
...
const normalizePortoHeader = (cleanKey: string): string => {
  ...
  if (cleanKey.includes("dia") && cleanKey.includes("semana")) {
    ...
    return "modelo";
  } else if (cleanKey.includes("tag") || cleanKey.includes("placa")) {
    return "placa";
    ...
    return "telefone";
  } else if (cleanKey.includes("dataprogramada") || cleanKey.includes("data")) {
    return "dataprogramada";
  } else if (cleanKey.includes("horainicio") || cleanKey.includes("inicio")) {
    return "horainicio";
  } else if (cleanKey.includes("horafim") || cleanKey.includes("fim")) {
    return "horafim";
    ...
    return "os";
  } else if (cleanKey.includes("sublocal")) {
    return "subet";
...
function PortoOperacaoPage() {
  ...
  // 2x2 rotation config (stored in localStorage, default to 2026-06-02 / A)
  ...
  const [detailsSchedule, setDetailsSchedule] = useState<PortoSchedule | null>(null);

  const [editOpen, setEditOpen] = useState(false);
  const [editingSchedule, setEditingSchedule] = useState<PortoSchedule | null>(null);
  ...
  const getPessoaByPlaca = (placa: string, dateStr: string, turno: string): any | null => {
    ...
    return people.find(p =>
      ...
      (!p.contrato || p.contrato === "CCO PORTO")
    ) || null;
  };
  ...
  const loadData = async () => {
    // 1. Carrega equipamentos de forma independente (para não falhar caso as tabelas da porto não existam)
    try {
    ...
    try {
      ...
      if (dayScheds.length === 0) {
        ...
        if (templatesForDay.length > 0) {
          ...
          localStorage.setItem("local_porto_schedules", JSON.stringify(mergedScheds));
        }
      }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Find the index of the header row (typically row 1 or 2, looking for 'OS', 'Dono', or 'TAG')
        let headerRowIdx = 0;
        for (let i = 0; i < Math.min(rawRows.length, 10); i++) {
          const row = rawRows[i] || [];
          const hasOS = row.some(cell => /os|ordem/i.test(String(cell)));
          const hasDono = row.some(cell => /dono/i.test(String(cell)));
          const hasTAG = row.some(cell => /tag/i.test(String(cell)));
          if (hasOS && (hasDono || hasTAG)) {
            headerRowIdx = i;
            break;
          }
        }
  ...
  const handleImportSchedules = async () => {
    ...
    for (const row of previewData) {
      ...
      if (resolvedLocal) {
        plateLocalMap[rawPlaca] = resolvedLocal;
      }
      ...
      if (isNightShift && rawOS.includes("/")) {
        ...
        const basePayload = {
          ...
          cost_center: row.centrodecusto || null,
          subet: row.subet || null,
          local: resolvedLocal || plateLocalMap[rawPlaca] || null,
          activity: row.atividade || null,
          operator: row.operador || null,
          phone: row.telefone ? row.telefone.toString().trim() : null,
          is_completed: false,
          owner_id: user?.id
        };
        ...
        if (valley_start) {
         ...
         finalPayloads.push({
          ...
          model: row.modelo || null,
          client: row.cliente || null,
          shift: isNightShift ? "NOITE" : (row.turno || "DIA"),
  ...
  const handleAddCorrective = async () => {
    ...
    try {
      ...
      localStorage.setItem("local_porto_corrective_logs", JSON.stringify(localLogs));
  ...
  const handleFinishStop = async (logId: string, endTime: string) => {
    ...
    try {
      ...
      localStorage.setItem("local_porto_corrective_logs", JSON.stringify(mapped));
  ...
  const handleSaveEditStop = async () => {
    ...
    try {
      ...
      localStorage.setItem("local_porto_corrective_logs", JSON.stringify(updated));
  ...
  const handleDeleteStop = async (logId: string) => {
    try {
      ...
      localStorage.setItem("local_porto_corrective_logs", JSON.stringify(filtered));
  ...
  const handleUpdateSchedule = async () => {
    ...
    try {
      ...
      localStorage.setItem("local_porto_schedules", JSON.stringify(updated));
      toast.success("Atualizado localmente");
    }
          ...
          <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
            <div className="bg-slate-50 border-b p-3 flex justify-between items-center flex-wrap gap-2">
              <div className="flex items-center gap-3">
                <h2 className="font-black text-slate-800 text-xs uppercase tracking-wider">Programação Operacional Porto</h2>
                ...
                <Dialog open={importOpen} onOpenChange={setImportOpen}>
                  ...
                  <DialogContent className={previewData.length > 0 ? "max-w-4xl max-h-[90vh] overflow-y-auto" : "max-w-md"}>
                    <DialogHeader>
                      ...
                      <DialogDescription className="sr-only">Selecione uma planilha excel (.xlsx, .xls) ou PDF para importar a programação diária da porto.</DialogDescription>
                    </DialogHeader>
